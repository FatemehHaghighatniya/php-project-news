            ...
            <tbody>
              ...
              foreach($posts as $post){?>
               ...
               <?=$post['id']?>
                    ...
                    <?=$post['title']?>
                    <td>
                    <?=substr($post['summary'] , 0 , 50)?> </td>
                    <td>
                    <?=$post['view']?>
                    </td>
                    <td>
                        <?= $post['breaking_news'] == 1 ? '<span class="badge badge-success">#breaking_news</span>' : '' ?><br>
                ...
                <td style="width: 25rem;">
                    ...
                    <a role="button" class="btn btn-sm btn-warning btn-dark text-white" href="<?=url('admin/post/breaking-news/' . $post['id'])?>"><?=$post['breaking_news'] == 1 ? 'remove breaking news' : 'add breaking news'?>
                    
                        </a>
                        <a role="button" class="btn btn-sm btn-warning btn-dark text-white" href="<?=url('admin/post/selected/' .$post['id'])?>"><?=$post['selected'] == 1 ? 'remove selcted' : 'add selected' ?>
